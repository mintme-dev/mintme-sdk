...
function revokeAuthority(options) {
  ...
  return idlUtils
    ...
    .then(async (idl) => {
      ...
      // Now using the RevokeFeeConfig PDA instead of NetworkFeeConfig
      // Derive the PDA addresses
      const [networkConfigPDA] = await PublicKey.findProgramAddress([Buffer.from("network_fee_config")], programId)
      const [revokeConfigPDA] = await PublicKey.findProgramAddress([Buffer.from("revoke_fee_config")], programId) // Añadir esta línea
      ...
      return program.methods
        .revokeAuthority(revokeMint, revokeFreeze)
        .accounts({
          ...
          config: revokeConfigPDA, // Usar revokeConfigPDA en lugar de configPDA
        })
        .rpc()
        .then((txSignature) => ({
          success: true,
