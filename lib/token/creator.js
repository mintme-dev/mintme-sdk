  ...
  return connection
    ...
    .then((balance) => {
      ...
      return createToken({
        ...
        logger: logger, // Pasar el logger a createToken
      });
    })
    .then((result) => {
      tokenResult = result;
      ...
      if (result.success && config.revokeFreeze) {
        logger("Revoking freeze authority...");
        ...
        return tokenAuthority.revokeFreezeAuthority({
          ...
          symbol: config.tokenSymbol,
          uniqueKey: config.uniqueKey,
          logger: logger, // Pasar el logger a revokeFreezeAuthority si lo soporta
        });
      }
...
module.exports = {
  createToken,
  createTokenSimple,
  setCustomLogger // Exportar la función para establecer el logger
};